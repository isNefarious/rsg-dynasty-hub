...
def load_sheet_tab(sheet_name):
    ...
    try:
        ...
        if 'html' in str(df.columns).lower() or df.empty:
            st.error(f"⚠️ Permissions Error reading '{sheet_name}'. Ensure Google Sheet is set to 'Anyone with the link can view'.")
            return pd.DataFrame()
        ...
        # Normalize year data to integers
        for col in df.columns:
            if col.lower().strip() == 'year' or col.lower().strip() == 'current year':
                df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(int)
        return df
    except Exception as e:
        st.error(f"⚠️ Could not load '{sheet_name}'. (Error: {e})")
        return pd.DataFrame()
...
def get_team_logo(team_name):
    ...
    logo_map = {
        "UMASS": "113", "OHIO STATE": "194", "INDIANA": "84", "PITTSBURGH": "221", "PITT": "221",
        "SMU": "2567", "GEORGIA": "61", "TEXAS A&M": "245", "BYU": "252",
        "MIAMI (OH)": "193", "MIAMI": "2390", "W. MICHIGAN": "2711", "BUFFALO": "2084",
        "E. MICHIGAN": "251", "OHIO": "195", "SAC STATE": "16", "BOWLING GREEN": "189",
        "BALL STATE": "2050", "TOLEDO": "2649", "AKRON": "2006", "C. MICHIGAN": "2117",
        "KENT STATE": "2309", "UNLV": "2439", "TEXAS TECH": "2641", "NOTRE DAME": "87",
        "OREGON": "2483", "SOUTH ALABAMA": "6", "GEORGIA SOUTHERN": "290", "WESTERN KENTUCKY": "98",
        
        "SOUTH CAROLINA": "2579", "UTEP": "2638", "MEMPHIS": "235", 
        "SOUTHERN MISS": "2572", "SOUTHERN MISSISSIPI": "2572", "SOUTHERN MISSISSIPPI": "2572",
        "GEAORGIA SOUTHERN": "290", "ALABAMA": "333", "BOISE STATE": "68", "LSU": "99", 
        "MINNESOTA": "135", "MISSOURI": "142", "NEBRASKA": "158", "PENN STATE": "213", 
        "SAN DIEGO ST": "21", "SAN DIEGO STATE": "21", "TENNESSEE": "2633", "UCLA": "26", "VIRGINIA": "258"
    }
...
# ==========================================
years_found = set()
for df in [df_top25, df_heisman, df_standings, df_bracket, df_dynasty, df_coaches, df_nil]:
    year_col = find_col(df, "year") if not df.empty else None
    if year_col:
...
with tab_coach:
    ...
    if not df_coaches.empty and year_col:
        ...
        if coach_col and not coach_df_year.empty:
            ...
            st.markdown("<br>", unsafe_allow_html=True)
            details_l, details_r = st.columns(2)
            ...
            with details_r:
                st.markdown("### 🎖️ Legacy Milestones")
                legacy_html = (
                    '<div class="sports-card" style="border-left-color: #d97706;">'
                    '<table style="width:100%; border:none; background:none;">'
                    f'<tr style="border:none; background:none;"><td style="padding:6px; font-weight:bold; color:#9ca3af; border:none;">PRO DRAFT SELECTIONS:</td><td style="padding:6px; font-weight:bold; text-align:right; border:none; color:white;">{draft} Players</td></tr>'
                    f'<tr style="border:none; background:none;"><td style="padding:6px; font-weight:bold; color:#9ca3af; border:none;">TOP 5 RECRUITING CLASSES:</td><td style="padding:6px; font-weight:bold; text-align:right; border:none; color:white;">{t5rec} Classes</td></tr>'
                    '</table></div>'
                )
                st.markdown(legacy_html, unsafe_allow_html=True)
        else:
...
# ==========================================
# TAB 3: 🛡️ TEAM & NIL TRACKER
# ==========================================
with tab_team:
    st.markdown("### 💰 NIL & Salary Cap Compliance Tracker")
    
    if not df_roster.empty and not df_nil.empty:
        r_year_col = find_col(df_roster, "year")
        team_col = find_col(df_roster, "team")
        
        roster_year_df = df_roster[df_roster[r_year_col] == selected_year]
        if not roster_year_df.empty and team_col:
            teams_avail = sorted(list(roster_year_df[team_col].dropna().unique()))
            selected_team = st.selectbox("🛡️ SELECT PROGRAM", options=teams_avail)
            
            team_roster = roster_year_df[roster_year_df[team_col] == selected_team]
            
            # Fetch NIL Data
            n_year_col = find_col(df_nil, "year")
            n_team_col = find_col(df_nil, "team")
            team_nil = df_nil[(df_nil[n_year_col] == selected_year) & (df_nil[n_team_col] == selected_team)]
            ...
            if not team_nil.empty:
                ...
                st.warning("No NIL Payroll logged for this team yet.")
            ...
            if display_cols:
                st.dataframe(team_roster[display_cols], use_container_width=True, hide_index=True, height=400)
        else:
            st.info("No roster data logged for this season yet.")
    else:
        st.info("Roster and NIL sheets are awaiting data.")
...
with tab_bracket:
    ...
    if not df_bracket.empty:
        ...
        if not year_bracket.empty:
            # Create a helper to find a row by round and specific position
            # This makes your code work even if your Game_IDs change
